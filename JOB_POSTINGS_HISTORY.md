# Job Postings History

This file tracks the history of job postings on the Escose Technologies Careers page.

## Last Updated
**Date:** November 18, 2025

---

## Currently Active Job Postings

### 1. Python Developer
- **Job ID:** ESC2511
- **Department:** Engineering
- **Location:** Remote / India
- **Type:** Full-time
- **Experience:** 3–5 years
- **Status:** Active (kept from original postings)

### 2. Business Analyst (BA)
- **Job ID:** ESC2514
- **Department:** Product & AI
- **Location:** HSR Layout, Bengaluru
- **Type:** Full-time
- **Experience:** 1–3 years
- **Openings:** 5
- **Posted Date:** 18 Nov 2025
- **Status:** Active (added today)

### 3. Prompt Engineer (LLM / GenAI)
- **Job ID:** ESC2515
- **Department:** AI & Data
- **Location:** HSR Layout, Bengaluru
- **Type:** Full-time
- **Experience:** 1–4 years
- **Openings:** 10
- **Posted Date:** 18 Nov 2025
- **Status:** Active (added today)

### 4. Quality Assurance (QA) Engineer for AI product
- **Job ID:** ESC2516
- **Department:** AI & Data
- **Location:** HSR Layout, Bengaluru
- **Type:** Full-time
- **Experience:** 1–5 years
- **Openings:** 6
- **Posted Date:** 18 Nov 2025
- **Status:** Active (added today)

### 5. DevOps Engineer
- **Job ID:** ESC2517
- **Department:** Engineering
- **Location:** UB City & Bellandur, Bengaluru
- **Type:** Full-time
- **Experience:** 2–5 years
- **Openings:** 7
- **Posted Date:** 18 Nov 2025
- **Status:** Active (added today)

### 6. Full Stack Engineer (Node + React)
- **Job ID:** ESC2518
- **Department:** Engineering
- **Location:** KR Puram, Bengaluru
- **Type:** Full-time
- **Experience:** 2–5 years
- **Openings:** 10
- **Posted Date:** 18 Nov 2025
- **Status:** Active (added today)

---

## Removed Job Postings

### 1. Java Developer
- **Job ID:** ESC2510
- **Department:** Engineering
- **Location:** Remote / India
- **Type:** Full-time
- **Experience:** 3–5 years
- **Removed Date:** November 18, 2025
- **Reason:** Older posting, removed to keep only Python Developer from original postings

### 2. AI/ML Developer
- **Job ID:** ESC2512
- **Department:** AI & Data
- **Location:** Remote / India
- **Type:** Full-time
- **Experience:** 4–5 years
- **Removed Date:** November 18, 2025
- **Reason:** Older posting, removed to keep only Python Developer from original postings

### 3. GenAI / Agentic Developer
- **Job ID:** ESC2513
- **Department:** AI & Data
- **Location:** Remote / India
- **Type:** Full-time
- **Experience:** 4–5 years
- **Removed Date:** November 18, 2025
- **Reason:** Older posting, removed to keep only Python Developer from original postings

---

## Job ID Sequence

Job IDs follow the pattern: **ESC25XX** where XX is an incrementing number.

### Used Job IDs:
- ESC2510 - Java Developer (removed)
- ESC2511 - Python Developer (active)
- ESC2512 - AI/ML Developer (removed)
- ESC2513 - GenAI / Agentic Developer (removed)
- ESC2514 - Business Analyst (BA) (active)
- ESC2515 - Prompt Engineer (LLM / GenAI) (active)
- ESC2516 - Quality Assurance (QA) Engineer for AI product (active)
- ESC2517 - DevOps Engineer (active)
- ESC2518 - Full Stack Engineer (Node + React) (active)

### Next Available Job ID:
**ESC2519**

---

## Notes

- All new job postings added on November 18, 2025 include:
  - Job ID
  - Posted date (18 Nov 2025)
  - Number of openings
  - Custom application URL (Google Form)

- Python Developer was kept from the original postings as it was specifically requested to be retained.

- Application form for new postings: https://forms.gle/cczcwqa7xpKzLjio9

---

## Future Updates

When adding or removing job postings, please update this file with:
1. Job details (ID, title, department, location, etc.)
2. Date added/removed
3. Reason for removal (if applicable)
4. Update the "Last Updated" date at the top

